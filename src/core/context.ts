...
export class OhlcvSeries {
  public readonly open = createSeries<number>(); public readonly high = createSeries<number>();
  public readonly low = createSeries<number>(); public readonly close = createSeries<number>();
  public readonly volume = createSeries<number>(); public readonly time = createSeries<number>();
  public readonly hl2 = createSeries<number>(); public readonly hlc3 = createSeries<number>();
  public readonly ohlc4 = createSeries<number>();

  public commit(bar: Bar): void {
    this.open.push(bar.open); this.high.push(bar.high); this.low.push(bar.low); this.close.push(bar.close);
    this.volume.push(bar.volume); this.time.push(bar.time);
    this.hl2.push((bar.high + bar.low) / 2); this.hlc3.push((bar.high + bar.low + bar.close) / 3);
  ...
  public replaceCurrent(bar: Bar): void {
    ...
    this.hl2.replaceCurrent((bar.high + bar.low) / 2); this.hlc3.replaceCurrent((bar.high + bar.low + bar.close) / 3);
